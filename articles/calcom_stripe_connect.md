...
- API v2（Platform向けConnect導線）
  - [`apps/api/v2/src/modules/stripe/controllers/stripe.controller.ts`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/apps/api/v2/src/modules/stripe/controllers/stripe.controller.ts)
  - [`apps/api/v2/src/modules/stripe/stripe.service.ts`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/apps/api/v2/src/modules/stripe/stripe.service.ts)
  - [`apps/api/v2/src/modules/organizations/stripe/organizations-stripe.controller.ts`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/apps/api/v2/src/modules/organizations/stripe/organizations-stripe.controller.ts)
  - [`apps/api/v2/src/modules/organizations/stripe/services/organizations-stripe.service.ts`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/apps/api/v2/src/modules/organizations/stripe/services/organizations-stripe.service.ts)
- Front（Platform Atoms）
  - [`packages/platform/atoms/hooks/stripe/useConnect.ts`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/packages/platform/atoms/hooks/stripe/useConnect.ts)
  - [`packages/platform/atoms/hooks/stripe/useCheck.ts`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/packages/platform/atoms/hooks/stripe/useCheck.ts)
  - [`packages/platform/atoms/connect/stripe/StripeConnect.tsx`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/packages/platform/atoms/connect/stripe/StripeConnect.tsx)
- App Store（旧来のStripe導線）
  - [`packages/app-store/stripepayment/api/add.ts`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/packages/app-store/stripepayment/api/add.ts)
  - [`packages/app-store/stripepayment/api/callback.ts`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/packages/app-store/stripepayment/api/callback.ts)
  - [`packages/app-store/_utils/oauth/createOAuthAppCredential.ts`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/packages/app-store/_utils/oauth/createOAuthAppCredential.ts)
- 決済実行・Webhook
  - [`packages/app-store/stripepayment/lib/PaymentService.ts`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/packages/app-store/stripepayment/lib/PaymentService.ts)
  ...
  - [`apps/web/pages/api/integrations/stripepayment/webhook.ts`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/apps/web/pages/api/integrations/stripepayment/webhook.ts)
...
[`packages/app-store/stripepayment/lib/PaymentService.ts`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/packages/app-store/stripepayment/lib/PaymentService.ts) で、保存済みキー
...
決済画面 ([`Payment.tsx`](https://github.com/calcom/cal.com/blob/5d65a0f09199abeab9729d2665e9bed399692f55/packages/features/ee/payments/components/Payment.tsx)) は `payment.data.stripe_publishable_key` を `loadStripe()` に渡して Stripe Elements を初期化します。
...
#### 6.2.1 Webhookで受け取るイベント

`webhookHandlers` で明示的に処理されるのは、次の2イベントです。

- `payment_intent.succeeded`
  - 即時決済（PaymentIntent）の成功通知
- `setup_intent.succeeded`
  - HOLD系フロー（SetupIntent）の成功通知

それ以外の `event.type` は「未処理イベント」として `202` を返し、Stripe側には受領済みとして返答します。
...
1. `POST` 以外は拒否
2. `stripe-signature` ヘッダを必須チェック（なければ `400`）
3. raw body を `buffer(req)` で読み取り、`stripe.webhooks.constructEvent(...)` で署名検証（secret未設定は `500`）
4. `event.account` と `NEXT_PUBLIC_IS_E2E` を使って connected account 判定を実施
5. `event.type` を `webhookHandlers` でディスパッチ
6. ハンドラがなければ `202`（Unhandled）を返す

#### 6.2.3 イベントごとの処理

- `payment_intent.succeeded`
  1. `paymentIntent.id` を `Payment.externalId` として Payment を検索
  2. Payment や `bookingId` がなければ `204`
  3. `handlePaymentSuccess({ appSlug: "stripe", paymentId, bookingId })` を実行
  4. ここで Payment 成功反映、`booking.paid` 更新、確認不要予約は `ACCEPTED` への遷移、`BOOKING_PAID` 系のWebhook/ワークフロー実行、必要に応じて `handleConfirmation` / `handleBookingRequested` へ進む

- `setup_intent.succeeded`
  1. `setupIntent.id` で Payment を検索
  2. Payment 不在や必要データ欠落時は `204`
  3. `getBooking(...)` で予約・主催者情報を取得し、確認要否（`requiresConfirmation`）を評価
  4. 確認不要なら `EventManager.create(...)` でカレンダー作成し、予約を `ACCEPTED` に更新
  5. Payment 側には `setupIntent` 情報を保存しつつ、予約の `paid` を更新
  6. 確認不要なら `handleConfirmation`、確認必要ならリクエスト通知メール送信へ進む
